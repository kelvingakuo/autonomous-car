...
class TrainLaneFollower(object):
	def __init__(self):
		self.model = lane_follower_v2.laneFollower()
	...
	def trainModel(self, XTrain, yTrain, XTest, yTest):
		...
		self.model.fit_generator(self.generator(XTrain, yTrain, batch_size = 32), steps_per_epoch = len(XTrain) / 32, epochs = 200, validation_data = self.generator(XTest, yTest), validation_steps = 200, verbose = 1, shuffle = 1, callbacks = [self.saveModelIfImprove, self.lossLogs])
